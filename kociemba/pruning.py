...
def create_phase1_prun_table():
    ...
    fname = path.join(relative_path, "phase1_prun")
...
def create_phase2_prun_table():
    ...
    fname = path.join(relative_path, "phase2_prun")
    ...
    if not path.isfile(fname):
        ...
        with open(fname, "wb") as fh:
            np.save(fh, corners_ud_edges_depth3, allow_pickle=False)
    else:
        # print("loading " + fname + " table...")
        with open(fname, "rb") as fh:
...
def create_phase2_cornsliceprun_table():
    ...
    fname = path.join(relative_path, "phase2_cornsliceprun")
    ...
    if not path.isfile(fname):
        ...
        print()
        with open(fname, "wb") as fh:
            np.save(fh, cornslice_depth, allow_pickle=False)
    else:
        # print("loading " + fname + " table...")
        with open(fname, "rb") as fh:
